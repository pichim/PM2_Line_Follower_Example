...
// velocity controllers functions
...
int main()
{
    ...
    // sleep - to wait for the signal from the environment - line detection
    enum RobotState {
        INITIAL,
        FOLLOW,
        SLEEP,
    ...
    // condition for state machine that will stop the robot 1 seconds after leaving the line
        ...
        if (do_execute_main_task) {
            ...
            switch (robot_state) {
                ...
                case RobotState::FOLLOW: {
                    motor_M1.setVelocity(wheel_speed(0) / (2.0f * M_PI)); // set a desired speed for right motor
                    motor_M2.setVelocity(wheel_speed(1) / (2.0f * M_PI)); // set a desired speed for left motor
...
// function to calculate rotational velocity of vechicle
float ang_cntrl_fcn(const float& Kp, const float& Kp_nl, const float& angle)
{
...
// function to calculate velocity of each wheel
float vel_cntrl_v2_fcn(const float& wheel_speed_max, const float& b, const float& robot_omega, const Eigen::Matrix2f& Cwheel2robot)
{
